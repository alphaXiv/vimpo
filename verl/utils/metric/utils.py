...
def _metric_reduction_hint(key: str) -> str | None:
    """Infer explicit min/max reductions from the metric name tail."""
    tail = key.rsplit("/", 1)[-1]
    if tail in {"min", "max"}:
        return tail
    if tail.endswith("_min"):
        return "min"
    if tail.endswith("_max"):
        return "max"
    return None
...
def reduce_metrics(metrics: dict[str, Union["Metric", list[Any]]]) -> dict[str, Any]:
    ...
    - If the metric tail is "max" or ends with "_max", np.max is used
    - If the metric tail is "min" or ends with "_min", np.min is used
    ...
    Example:
        ...
        {"loss": 2.0, "accuracy": 0.8, "reward_max": 8.0, "error_min": 0.05}
    """
    for key, val in metrics.items():
        if isinstance(val, Metric):
            metrics[key] = val.aggregate()
        else:
            reduction_hint = _metric_reduction_hint(key)
            if reduction_hint == "max":
                metrics[key] = np.max(val)
            elif reduction_hint == "min":
