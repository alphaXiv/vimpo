...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            colocated_ref_worker = self.get_fused_worker_by_name("ref")
            colocated_ref_policy = None
            if colocated_ref_worker is not None:
                colocated_ref_policy = getattr(colocated_ref_worker, "ref_policy", None)

            if self._is_lora:
                # In LoRA mode, the actor with adapters disabled acts as the
                # reference policy, so we do not need a separate ref worker.
                self.actor.ref_policy = None
                self.actor._use_lora_reference = True
            else:
                self.actor.ref_policy = self.ref_policy if self._is_ref else colocated_ref_policy
                self.actor._use_lora_reference = False
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_ref_policy_from_actor(self):
        """Refresh the colocated reference policy from the current actor weights."""
        assert self._is_actor
        if self._is_lora:
            raise NotImplementedError("Updating the VIMPO reference policy from actor is not supported for LoRA models.")

        if getattr(self.actor, "ref_policy", None) is None:
            raise RuntimeError("Cannot update reference policy because no colocated reference worker was found.")

        ref_module = self.actor.ref_policy.actor_module
        ref_worker = self if self._is_ref else self.get_fused_worker_by_name("ref")
        ref_param_offload = bool(getattr(ref_worker, "_is_offload_param", False))

        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if ref_param_offload:
            load_fsdp_model_to_gpu(ref_module)

        with torch.no_grad():
            ref_fsdp_version = fsdp_version(ref_module)
            if ref_fsdp_version == 1:
                ...
                with FSDP.state_dict_type(
                    ...
                    actor_state_dict = self.actor_module_fsdp.state_dict()
                with FSDP.state_dict_type(
                    ref_module,
                    StateDictType.FULL_STATE_DICT,
                    full_state_config,
                ):
                    ref_module.load_state_dict(actor_state_dict, strict=True)
            elif ref_fsdp_version == 2:
                actor_state_dict = get_fsdp_full_state_dict(
                    self.actor_module_fsdp,
                    offload_to_cpu=True,
                    rank0_only=True,
                )
                fsdp2_load_full_state_dict(
                    ...
                    actor_state_dict,
                    device_mesh=self.device_mesh,
                    cpu_offload=True if ref_param_offload else None,
                )
            else:
                actor_state_dict = self.actor_module_fsdp.state_dict()
                ref_module.load_state_dict(actor_state_dict, strict=True)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if not is_lora and "ref_log_prob" in outputs:
                tensors["ref_log_prob"] = outputs["ref_log_prob"]
            if not is_lora and "vimpo_old_token_kl" in outputs:
                tensors["vimpo_old_token_kl"] = outputs["vimpo_old_token_kl"]
