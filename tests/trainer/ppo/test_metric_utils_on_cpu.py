...
class TestReduceMetrics(unittest.TestCase):
    ...
    def test_reduce_metrics_explicit_min_max_hints(self):
        """Test that reduce_metrics honors explicit min/max suffixes and path tails."""
        metrics = {
            "reward_max": [5.0, 8.0, 6.0],
            "error_min": [0.1, 0.05, 0.2],
            "response_length/max": [32, 64, 16],
            "response_length/min": [32, 64, 16],
        }
        result = reduce_metrics(metrics)

        self.assertEqual(result["reward_max"], 8.0)
        self.assertEqual(result["error_min"], 0.05)
        self.assertEqual(result["response_length/max"], 64)
        self.assertEqual(result["response_length/min"], 16)

    def test_reduce_metrics_does_not_treat_terminal_as_min(self):
        """Test that ordinary words containing min/max still reduce by mean."""
        metrics = {
            "actor/vimpo_terminal_squared_error_mean": [0.0, 2.0, 4.0],
            "actor/vimpo_terminal_value": [-1.0, 0.0, 1.0],
        }
        result = reduce_metrics(metrics)

        self.assertEqual(result["actor/vimpo_terminal_squared_error_mean"], 2.0)
        self.assertEqual(result["actor/vimpo_terminal_value"], 0.0)
